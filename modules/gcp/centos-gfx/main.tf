...
resource "google_monitoring_dashboard" "gcent-dashboard" {
  count = var.gcp_ops_agent_enable ? length(local.instance_info_list) : 0
  dashboard_json = <<EOF
{
  "category": "CUSTOM",
            ...
            "dataSets": [
              {
                ...
                "timeSeriesQuery": {
                  ...
                  "timeSeriesFilter": {
                    ...
                    "filter": "metric.type=\"compute.googleapis.com/instance/cpu/utilization\" resource.type=\"gce_instance\" resource.labels.instance_id=\"${google_compute_instance.centos-gfx[count.index].instance_id}\" "
                  }
                }
              }
            ],
